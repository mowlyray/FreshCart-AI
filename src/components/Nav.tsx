...
function Nav({user}:{user:IUser}) {
  ...
  const sidebar = menuOpen?createPortal(
    <AnimatePresence>
      ...
      className='fixed top-0 left-0 w-[75%] sm:w-[60%] h-full bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 z-9999 p-6 flex flex-col shadow-[0_0_50px_-10px_rgba(0,2555,100,0.3)] text-white'
      >
        ...
        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/15 transition-all shadow-inner'>

          <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 p-2.5 border-green-400/60 shadow-lg'>
        ...
        <div className='flex items-center gap-3 p-3 rounded-lg text-red-300 font-semibold mt-auto cursor-pointer hover:bg-red-500/20 transition-all' onClick={async ()=>await signOut({callbackUrl:"/"})}>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
        ...
        <div className="md:hidden bg-white rounded-full w-10 h-10 p-2 flex items-center  shadow-md" 
        ...
        <AnimatePresence>
         ...
         className='fixed top-24 left-1/2 -translate-x-1/2 w-[90%] bg-white rounded-full shadow-lg z-40 flex items-center px-4 py-2'
          >
            ...
            <button onClick={()=>setSearchBarOpen(false)} className='bg-red-100 p-1 rounded-full hover:bg-red-200 transition'>
